...
export default {
    ...
    created() {
        ...
        this.data = JSON.parse(JSON.stringify(this.lxsData.data ? this.lxsData.data : data));
        
        if (this.lxsData.data) {
            console.log('用户传的值', this.lxsData.data);
        } else {
            console.log('默认值', data);
        }

        this.data.rightData.navList.forEach((item) => {
            if (this.data.rightData.isShowView == item.id) {
